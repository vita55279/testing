...
async function startCooccurPage() {
	...
	slider.addEventListener('input', function() {
        ...
        document.getElementById('range-value').value = this.value;
		d3.selectAll(".ribbon")
			.style("opacity", d => {
				if (selectedIndex < 0) {
					...
					return (isConnected && isAboveThreshold) ? opacityScale(d.source.value) : 0.02; // 0.9
				}
			})
	...
	box.addEventListener('click', resetDiagram);
...
//      .attr("transform", `translate(${width/2}, ${height/2})`);
	...
	ribbonGroup.style("fill-opacity", 0.67)
		.selectAll("path")
//		.data(chords.map(d => {
//			const scaleArcMiddle = (arc) => {
//				const originalWidth = arc.endAngle - arc.startAngle;
//				const targetWidth = originalWidth * 0.6;
//				const padding = (originalWidth - targetWidth) / 2;
//				
//				return {
//					...arc,
//					startAngle: arc.startAngle + padding,
//					endAngle: arc.endAngle - padding
//				};
//			};
//			
//			return {
//				...d,
//				source: scaleArcMiddle(d.source),
//				target: scaleArcMiddle(d.target)
//			};
//		}))
//		.data(chords)
		.data(chords.map(d => {
...
function resetDiagram() {
	...
	d3.selectAll(".ribbon")
		...
		.duration(200)
		.style("opacity", d => {
			return d.source.value >= mid ? opacityScale(d.source.value) : 0 // 0.7
		})
...
async function renderTopPairs() {
    ...
    topPairs.forEach(p => {
		...
		el.innerHTML = `
            <div class="pair-left">
                <div class="pair-ball purple">${p.num1}</div>
                <span class="pair-arrow">↔</span>
                <div class="pair-ball blue">${p.num2}</div>
            </div>
            <div class="pair-right">
                <div class="pair-count">${p.value}</div>
